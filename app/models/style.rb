class Style < ApplicationRecord
  ...
  def self.filter_by_category(category_id, styles)
    return styles unless category_id.present?
    category = Category.find_by(id: category_id)
    return styles unless category.present?
    styles.joins(:sub_category).where(sub_categories: { id: category.sub_categories.ids })
  end

  def self.filter_by_gender(gender, styles)
    styles = styles.where("gender ILIKE ?", "#{gender}") if gender.present?
    styles
  end
