...
class ColorsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create color" do
    assert_difference("Color.count") do
      post colors_url, params: { color: { color_code: @color.color_code, name: @color.name, variant_id: @color.variant_id } }
    end
  ...
  test "should update color" do
    patch color_url(@color), params: { color: { color_code: @color.color_code, name: @color.name, variant_id: @color.variant_id } }
